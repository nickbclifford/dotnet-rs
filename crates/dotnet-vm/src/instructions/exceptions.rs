...
pub fn leave<'gc, 'm: 'gc, T: ExceptionOps<'gc>>(ctx: &mut T, jump_target: usize) -> StepResult {
...
pub fn throw<'gc, 'm: 'gc, T: TypedStackOps<'gc> + ExceptionOps<'gc>>(ctx: &mut T) -> StepResult {
    let exc = ctx.pop_obj();
    if exc.0.is_none() {
