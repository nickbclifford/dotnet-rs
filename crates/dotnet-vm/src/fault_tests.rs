...
mod tests {
    ...
    use dotnetdll::{
        ...
        resolved::{
            ...
            members::{Method, UserMethod, ExternalMethodReference, MethodReferenceParent},
            signature::ReturnType,
            types::{ExternalTypeReference, ResolutionScope, TypeDefinition, LocalVariable, MethodType, BaseType},
        },
    };
    ...
    fn get_mock_loader() -> Arc<AssemblyLoader> {
        MOCK_LOADER
            .get_or_init(|| {
                ...
                system_runtime.push_method(obj_idx, Method::new(
                ...
                // Add empty .ctor to Exception
                system_runtime.push_method(exc_idx, Method::new(
                    resolved_mod::Accessibility::Public,
                    void_sig,
                    ".ctor",
                    Some(body::Method {
                ...
                // Register as mscorlib and System.Runtime too for compatibility
    ...
    fn test_fault_handler_skipped_on_normal_exit() {
        ...
        let sig = MethodSignature {
            ...
            calling_convention: CallingConvention::Default,
            parameters: vec![],
            return_type: ReturnType(vec![], Some(dotnetdll::prelude::ParameterType::Value(MethodType::Base(Box::new(BaseType::Int32))))),
            varargs: None,
        };
        ...
        let body = body::Method {
            ...
            instructions: vec![
                Instruction::LoadConstantInt32(42), // IP 0
                Instruction::StoreLocal(0), // IP 1
                Instruction::Leave(6), // IP 2. Absolute IP 6 is LoadLocal(0).
                Instruction::LoadConstantInt32(100), // IP 3 (fault handler)
                Instruction::StoreLocal(0), // IP 4
                Instruction::EndFinally, // IP 5
                Instruction::LoadLocal(0), // IP 6
                Instruction::Return, // IP 7
            ],
            data_sections: vec![
                body::DataSection::ExceptionHandlers(vec![
                    body::Exception {
                        ...
                        try_offset: 0,
                        try_length: 3, // IP 0, 1, 2
                        handler_offset: 3, // IP 3, 4, 5
                        handler_length: 3,
                    }
                ])
            ],
    ...
    fn test_fault_handler_executed_on_exception() {
        ...
        let exc_ctor = res.push_method_reference(ExternalMethodReference {
            parent: MethodReferenceParent::Type(MethodType::Base(Box::new(BaseType::class(exception_type_ref)))),
            name: ".ctor".into(),
            signature: ctor_sig,
            attributes: vec![],
        });
        ...
        let body = body::Method {
            ...
            instructions: vec![
                Instruction::NewObject(UserMethod::Reference(exc_ctor)), // IP 0
                Instruction::Throw, // IP 1
                Instruction::LoadConstantInt32(100), // IP 2 (fault handler)
                Instruction::StoreLocal(0), // IP 3
                Instruction::EndFinally, // IP 4
                Instruction::Pop, // IP 5 (catch handler)
                Instruction::LoadLocal(0), // IP 6
                Instruction::Return, // IP 7
            ],
            data_sections: vec![
                body::DataSection::ExceptionHandlers(vec![
                    body::Exception {
                        ...
                        try_offset: 0,
                        try_length: 2, // IP 0, 1
                        handler_offset: 2, // IP 2, 3, 4
                        handler_length: 3,
                    },
                    body::Exception {
                        kind: body::ExceptionKind::TypedException(MethodType::Base(Box::new(BaseType::class(exception_type_ref)))),
                        try_offset: 0,
                        try_length: 5, // IP 0, 1, 2, 3, 4
                        handler_offset: 5, // IP 5, 6, 7
                        handler_length: 3,
                    }
                ])
            ],
