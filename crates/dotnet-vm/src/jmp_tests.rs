...
mod tests {
    ...
    fn test_jmp_instruction() {
        ...
        let sig = MethodSignature {
            ...
            parameters: vec![dotnetdll::resolved::signature::Parameter(
                vec![],
                dotnetdll::prelude::ParameterType::Value(dotnetdll::resolved::types::MethodType::Base(Box::new(dotnetdll::resolved::types::BaseType::Int32))),
            )],
            return_type: ReturnType(vec![], Some(dotnetdll::prelude::ParameterType::Value(dotnetdll::resolved::types::MethodType::Base(Box::new(dotnetdll::resolved::types::BaseType::Int32))))),
            varargs: None,
        };

        // Target method: returns the argument + 10
        ...
        // Jumper method: just jumps to Target
        ...
        arena.mutate_root(|gc, engine| {
            let gc_handle = GCHandle::new(
                ...
                unsafe { engine.stack.arena_inner_gc() },
                #[cfg(feature = "memory-validation")]
                thread_id,
            );
            ...
            engine
                ...
                .entrypoint_frame(info, Default::default(), vec![dotnet_value::StackValue::Int32(42)])
        ...
        for _ in 0..1000 {
            ...
            match step_res {
                ...
                StepResult::MethodThrew(exc) => panic!("Unexpected managed exception: {exc}"),
                _ => {}
            }
        }

        assert_eq!(final_int, Some(52));
        // Entrypoint frame + Target frame (Jumper frame was replaced by Target)
        // Wait, entrypoint_frame pushes one frame. 
        // Then step 1: execution of Jumper starts.
        // step 2: Jump instruction executed. Jumper frame replaced by Target frame.
        // so max_depth should be 1.
    ...
    fn test_jmp_invalid_stack() {
        ...
        let target_idx = res.push_method(
            ...
            Method::new(
                ...
                "Target",
                Some(body::Method {
                    header: body::Header { maximum_stack_size: 1, local_variables: vec![], initialize_locals: true },
        ...
        // Jumper method: has something on stack before jmp
        let jumper_body = body::Method {
            header: body::Header {
        ...
        arena.mutate_root(|gc, engine| {
            let gc_handle = GCHandle::new(gc, #[cfg(feature = "multithreading")] unsafe { engine.stack.arena_inner_gc() }, #[cfg(feature = "memory-validation")] thread_id);
            let info = shared.caches.get_method_info(entrypoint, &Default::default(), shared.clone()).expect("Failed to resolve entrypoint");
            engine.ves_context(gc_handle).entrypoint_frame(info, Default::default(), vec![]).unwrap();
        });
        ...
        for _ in 0..10 {
            ...
            if matches!(step_res, StepResult::Error(_)) { break; }
        }

        match step_res {
            StepResult::Error(crate::error::VmError::Execution(crate::error::ExecutionError::Aborted(msg))) => {
    ...
    fn test_jmp_inside_try() {
        ...
        let jumper_body = body::Method {
            ...
            instructions: vec![
                Instruction::Jump(MethodSource::User(UserMethod::Definition(target_idx))),
                Instruction::Return, // Never reached but makes it easier to define ranges
                Instruction::Return, // Finally block
            ],
            data_sections: vec![
                body::DataSection::ExceptionHandlers(vec![
                    body::Exception {
                        kind: body::ExceptionKind::Finally,
                        try_offset: 0,
                        try_length: 1, // Covers the Jump instruction
                        handler_offset: 2,
                        handler_length: 1,
                    }
                ])
        ...
        match step_res {
            ...
            _ => panic!("Expected invalid control transfer error, got {:?}", step_res),
        }
    }
    ...
    fn test_jmp_signature_mismatch() {
        ...
        // Jumper method: has void signature, jumps to int signature
        let jumper_body = body::Method {
            header: body::Header {
                maximum_stack_size: 0,
                local_variables: vec![],
