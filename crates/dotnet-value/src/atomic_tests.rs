#[cfg(test)]
mod tests {
    use crate::{StackValue, LoadType, StoreType};
    ...
    fn test_atomic_load_store() {
        ...
        unsafe {
            ...
            // Int32
            StackValue::Int32(0x12345678).store_atomic(ptr, StoreType::Int32, AtomicOrdering::SeqCst);
            let val = StackValue::load_atomic(ptr, LoadType::Int32, AtomicOrdering::SeqCst);
            assert_eq!(val, StackValue::Int32(0x12345678));

            // Int64
            StackValue::Int64(0x123456789ABCDEF0).store_atomic(ptr, StoreType::Int64, AtomicOrdering::SeqCst);
            let val = StackValue::load_atomic(ptr, LoadType::Int64, AtomicOrdering::SeqCst);
            assert_eq!(val, StackValue::Int64(0x123456789ABCDEF0));

            // Float32
            StackValue::NativeFloat(1.25).store_atomic(ptr, StoreType::Float32, AtomicOrdering::SeqCst);
            ...
            // Float64
            StackValue::NativeFloat(1.23456789).store_atomic(ptr, StoreType::Float64, AtomicOrdering::SeqCst);
    ...
    fn test_concurrent_atomic_load_store() {
        ...
        let writer = thread::spawn(move || {
            ...
            while !stop_clone.load(Ordering::Relaxed) {
                unsafe {
                    StackValue::Int64(i).store_atomic(ptr, StoreType::Int64, AtomicOrdering::Release);
                }
                i += 1;
            }
        });
        ...
        let reader = thread::spawn(move || {
            ...
            while !stop_clone2.load(Ordering::Relaxed) {
                unsafe {
                    let val = StackValue::load_atomic(ptr, LoadType::Int64, AtomicOrdering::Acquire);
                    if let StackValue::Int64(v) = val {
                        // Just ensure it's a valid value (no partial writes)
                        // In a real test we might want to ensure monotonicity if we only had one reader/writer
                        // but here we just check it doesn't crash or return garbage.
                        assert!(v >= 0);
                    }
                }
            }
    ...
    fn test_atomic_object_load_store() {
        ...
        arena.mutate(|gc, _root| {
            ...
            unsafe {
                StackValue::ObjectRef(obj).store_atomic(ptr, StoreType::Object, AtomicOrdering::SeqCst);
                ...
                StackValue::ObjectRef(ObjectRef(None)).store_atomic(ptr, StoreType::Object, AtomicOrdering::SeqCst);
                let val = StackValue::load_atomic(ptr, LoadType::Object, AtomicOrdering::SeqCst);
                if let StackValue::ObjectRef(read_obj) = val {
