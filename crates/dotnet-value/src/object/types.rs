...
// SAFETY: F5.TracesEveryGcRef — ValueType is an enum where only Pointer and Struct variants contain
...
// SAFETY: F5.TracesEveryGcRef — Vector contains raw byte storage that may hold GC pointers (ObjectRef).
        ...
        match element.as_ref() {
            LayoutManager::Scalar(Scalar::ObjectRef) => {
                ...
                for i in 0..self.layout.length {
                    // SAFETY: F2.DescriptorMatchesEcmaLayout — The object layout selected valid serialized storage for this value and preserves its lifetime.
                    unsafe {
...
// SAFETY: F5.TracesEveryGcRef — Object contains field storage that may hold GC pointers.
...
impl<'gc> Object<'gc> {
    pub fn size_bytes(&self) -> usize {
        // SAFETY: F10.BorrowedStorageStable — The object layout selected valid serialized storage for this value and preserves its lifetime.
