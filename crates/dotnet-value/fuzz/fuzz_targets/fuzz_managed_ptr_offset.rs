...
fuzz_target!(|data: (ManagedPtr<'static>, i32)| {
    ...
    if let Some(addr) = ptr.clone().into_info().address {
    ...
    assert_eq!(new_ptr.byte_offset().as_usize(), (initial_offset + offset_delta) as usize, "Offset mismatch");
    ...
    if let Some(orig_addr) = original_ptr.into_info().address {
        ...
        assert_eq!(new_ptr.into_info().address.map(|p| p.as_ptr() as usize), Some(expected_addr), "Address mismatch");
    }
});
