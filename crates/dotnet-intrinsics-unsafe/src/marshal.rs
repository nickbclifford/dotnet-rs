use crate::UnsafeIntrinsicHost;
use dotnet_macros::{dotnet_intrinsic, dotnet_intrinsic_field};
use dotnet_types::{
    generics::{ConcreteType, GenericLookup},
    members::{FieldDescription, MethodDescription},
};
use dotnet_value::{
    StackValue,
    layout::{FieldLayoutManager, HasLayout, LayoutManager},
};
use dotnet_vm_data::StepResult;
use dotnet_vm_ops::ops::{EvalStackOps, TypedStackOps};
use std::sync::Arc;

pub(super) fn offset_ptr<'gc>(val: StackValue<'gc>, byte_offset: isize) -> StackValue<'gc> {
    if let StackValue::ManagedPtr(m) = val {
        // SAFETY: F3.InteriorPointerRebased — This intrinsic's caller supplies the byte offset and preserves the managed
        // pointer's origin; `offset` only forms the corresponding managed address.
        let new_m = unsafe { m.into_inner().offset(byte_offset) };
        StackValue::ManagedPtr(new_m.into())
    } else {
        let ptr = val.as_ptr();
        // SAFETY: F10.RawMemoryAccessValid — The native pointer overload delegates its range/validity contract to the CLR
        // caller; this operation only applies that requested byte offset.
        let result_ptr = unsafe { ptr.offset(byte_offset) };
        StackValue::NativeInt(result_ptr as isize)
    }
}

fn compute_offset_and_layout<'gc, T: UnsafeIntrinsicHost<'gc>>(
    ctx: &mut T,
    layout: &FieldLayoutManager,
    field_name: &str,
) -> StepResult {
    if let Some(field) = layout.get_field_by_name(field_name) {
        ctx.push_isize(field.position.as_usize() as isize);
        StepResult::Continue
    } else {
        ctx.throw_by_name_with_message("System.ArgumentException", "Field not found.")
    }
}

#[dotnet_intrinsic("static int System.Runtime.InteropServices.Marshal::GetLastPInvokeError()")]
#[allow(unused_variables)]
pub fn intrinsic_marshal_get_last_pinvoke_error<'gc, T: UnsafeIntrinsicHost<'gc>>(
    ctx: &mut T,
    _method: MethodDescription,
    _generics: &GenericLookup,
) -> StepResult {
    let value = ctx.unsafe_get_last_pinvoke_error();
    ctx.push_i32(value);
    StepResult::Continue
}

#[dotnet_intrinsic("static void System.Runtime.InteropServices.Marshal::SetLastPInvokeError(int)")]
#[allow(unused_variables)]
pub fn intrinsic_marshal_set_last_pinvoke_error<'gc, T: UnsafeIntrinsicHost<'gc>>(
    ctx: &mut T,
    _method: MethodDescription,
    _generics: &GenericLookup,
) -> StepResult {
    let value = ctx.pop_i32();
    ctx.unsafe_set_last_pinvoke_error(value);
    StepResult::Continue
}

#[dotnet_intrinsic("static int System.Runtime.InteropServices.Marshal::SizeOf(object)")]
#[dotnet_intrinsic("static int System.Runtime.InteropServices.Marshal::SizeOf(System.Type)")]
#[dotnet_intrinsic("static int System.Runtime.InteropServices.Marshal::SizeOf<T>(T)")]
#[dotnet_intrinsic("static int System.Runtime.InteropServices.Marshal::SizeOf<T>()")]
pub fn intrinsic_marshal_size_of<'gc, T: UnsafeIntrinsicHost<'gc>>(
    ctx: &mut T,
    method: MethodDescription,
    generics: &GenericLookup,
) -> StepResult {
    let concrete_type = if method.signature().parameters.is_empty() {
        dotnet_vm_ops::vm_try!(generics.cloned_method_arg(0))
    } else {
        let type_obj = ctx.pop_obj();
        dotnet_vm_ops::vm_try!(ctx.unsafe_resolve_runtime_type(type_obj))
    };
    let layout = dotnet_vm_ops::vm_try!(ctx.unsafe_type_layout(concrete_type));
    ctx.push_i32(layout.size().as_usize() as i32);
    StepResult::Continue
}

#[dotnet_intrinsic(
    "static IntPtr System.Runtime.InteropServices.Marshal::OffsetOf(System.Type, string)"
)]
#[dotnet_intrinsic("static IntPtr System.Runtime.InteropServices.Marshal::OffsetOf<T>(string)")]
pub fn intrinsic_marshal_offset_of<'gc, T: UnsafeIntrinsicHost<'gc>>(
    ctx: &mut T,
    method: MethodDescription,
    generics: &GenericLookup,
) -> StepResult {
    use dotnet_value::with_string;
    let field_name_val = ctx.pop();
    let field_name = with_string!(ctx, field_name_val, |s| s.as_string());
    let concrete_type = if method.signature().parameters.len() == 1 {
        dotnet_vm_ops::vm_try!(generics.cloned_method_arg(0))
    } else {
        let type_obj = ctx.pop_obj();
        dotnet_vm_ops::vm_try!(ctx.unsafe_resolve_runtime_type(type_obj))
    };
    let layout = dotnet_vm_ops::vm_try!(ctx.unsafe_type_layout(concrete_type.clone()));

    if let LayoutManager::Field(flm) = &*layout {
        return compute_offset_and_layout(ctx, flm, &field_name);
    }

    if concrete_type.is_class(ctx.loader().as_ref()) {
        let td = dotnet_vm_ops::vm_try!(ctx.loader().find_concrete_type(concrete_type.clone()));
        let flm = dotnet_vm_ops::vm_try!(ctx.instance_field_layout(td));
        return compute_offset_and_layout(ctx, &flm, &field_name);
    }

    ctx.throw_by_name_with_message(
        "System.ArgumentException",
        "Type must be a structure or a class.",
    )
}

#[dotnet_intrinsic_field("static IntPtr System.IntPtr::Zero")]
pub fn intrinsic_field_intptr_zero<'gc, T: EvalStackOps<'gc> + TypedStackOps<'gc>>(
    ctx: &mut T,
    _field: FieldDescription,
    _type_generics: Arc<[ConcreteType]>,
    _is_address: bool,
) -> StepResult {
    ctx.push(StackValue::NativeInt(0));
    StepResult::Continue
}
