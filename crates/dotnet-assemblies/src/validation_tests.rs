...
mod tests {
    ...
    use dotnetdll::resolved::{
        Accessibility, members::Method, signature::ReturnType, types::TypeDefinition,
    };
    #[test]
    ...
    fn test_empty_method_name() {
        ...
        res.push_method(
            type_idx,
            Method::new(Accessibility::Public, void_sig, "", None),
        );
        let err = validate_metadata(&res).unwrap_err();
        assert!(
            err.to_string()
                .contains("method at index 0 with empty name")
        );
    }
}
