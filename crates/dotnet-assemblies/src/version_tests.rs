#[cfg(test)]
mod tests {
    use crate::loader::{AssemblyLoader, versions_equal, parse_version};
    ...
    fn test_version_equality() {
        let v1 = Version { major: 1, minor: 2, build: 3, revision: 4 };
        let v2 = Version { major: 1, minor: 2, build: 3, revision: 4 };
        let v3 = Version { major: 1, minor: 2, build: 3, revision: 5 };
    ...
    fn test_version_binding_basic() {
        ...
        assembly.version = Version { major: 1, minor: 0, build: 0, revision: 0 };
        res.assembly = Some(assembly);

        let res_s = loader.register_owned_assembly(res);

        // Exact match
        let resolved = loader.get_assembly_with_version("TestAssembly", Some(Version { major: 1, minor: 0, build: 0, revision: 0 })).unwrap();
        assert_eq!(resolved, res_s);

        // Newer version requested -> Should fail
        let err = loader.get_assembly_with_version("TestAssembly", Some(Version { major: 1, minor: 0, build: 0, revision: 1 }));
        assert!(err.is_err());

        // Major version mismatch -> Should fail
        let err = loader.get_assembly_with_version("TestAssembly", Some(Version { major: 2, minor: 0, build: 0, revision: 0 }));
        assert!(err.is_err());
    ...
    fn test_binding_redirect() {
        ...
        // Redirected match: 2.0.0.0 -> 1.0.0.0
        let resolved = loader.get_assembly_with_version("TestAssembly", Some(Version { major: 2, minor: 0, build: 0, revision: 0 })).unwrap();
        assert_eq!(resolved, res_s);
