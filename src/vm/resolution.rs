...
impl<'a, 'm> ValueResolution for ResolutionContext<'a, 'm> {
    ...
    fn new_instance_fields(&self, td: TypeDescription) -> FieldStorage {
        FieldStorage::new(FieldLayoutManager::instance_field_layout_cached(
            td, self, None,
        ))
    }

    fn new_static_fields(&self, td: TypeDescription) -> FieldStorage {
        FieldStorage::new(Arc::new(FieldLayoutManager::static_fields(td, self)))
    }
                    ...
                    if let StackValue::ObjectRef(o) = data {
                        ...
                        if let Some(handle) = o.0 {
                            ...
                            match &borrowed.storage {
                                crate::value::object::HeapStorage::Obj(obj) => {
    ...
    fn read_cts_value<'gc>(&self, t: &ConcreteType, data: &[u8]) -> CTSValue<'gc> {
        ...
        match t.get() {
            ...
            BaseType::Object | BaseType::String | BaseType::Vector(_, _) => {
                CTSValue::Ref(crate::value::object::ObjectRef::read(data))
            }
            BaseType::Type {
                value_kind: Some(ValueKind::Class),
                ..
    ...
    fn new_vector<'gc>(&self, element: ConcreteType, size: usize) -> Vector<'gc> {
        let layout = ArrayLayoutManager::new(element.clone(), size, self);
        ...
        Vector {
            storage: vec![0; total_size],
            layout,
            element,
            _contains_gc: std::marker::PhantomData,
        }
    }
}
...
fn convert_num<T: TryFrom<i32> + TryFrom<isize> + TryFrom<usize>>(data: StackValue<'_>) -> T {
    match data {
        ...
        StackValue::ManagedPtr(p) => (p.value.as_ptr() as usize)
            .try_into()
            .unwrap_or_else(|_| panic!("failed to convert from pointer")),
        other => panic!(
