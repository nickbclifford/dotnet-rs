...
pub type GCValueHandle = DynamicRoot<StackSlot>;
...
// TODO: I think the extra lifetime might be confusing things
pub type GCArena<'m> = Arena<Rootable!['gc => CallStack<'gc, 'm>]>;

impl<'gc, 'm> CallStack<'gc, 'm> {
    ...
    pub fn new_frame(&mut self, gc_handle: &'gc Mutation<'gc>, method: MethodInfo<'m>) {
        ...
        for l in method.locals {
            ...
            let handle = self
                .roots
                .stash(gc_handle, Gc::new(gc_handle, Cell::new(StackValue::null())));
    ...
    pub fn return_frame(&mut self, gc_handle: &'gc Mutation<'gc>) {
        ...
        // TODO: void methods?
        let return_value = self.roots.fetch(&self.stack[frame.base.stack]).get();

        for handle in &self.stack[frame.base.arguments..] {
            self.roots.fetch(handle).set(gc_handle, StackValue::null());
        }
    ...
    pub fn push_stack(&mut self, gc_handle: &'gc Mutation<'gc>, value: StackValue<'gc>) {
        ...
        match self.stack.get(f.base.stack + f.stack_height) {
            Some(h) => {
