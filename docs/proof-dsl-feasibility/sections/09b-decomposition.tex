...
\item \textbf{[B] \code{ves-tokens}. \textbf{Done, first pass}
  ...
  \item \emph{Trusted constructors are discovered by text-scraping.}
    The build script finds them by matching
    \code{\_grant: super::TrustGrant} on the same physical line as
    \code{pub fn}, which holds only because every trusted constructor
    today takes exactly one parameter and \code{rustfmt} keeps such
    signatures on one line. A trusted-\emph{and}-derived constructor
    would wrap and be silently skipped. The failure is fail-closed (the
    grant is not emitted, so the citing site does not compile), but it
    is a latent trap, and the underlying cause --- lane B re-deriving
    vocabulary facts from formatted Rust --- is what 6b removes.
  ...
  \end{enumerate}
  \smallskip
  \noindent\textbf{6b --- [B] Vocabulary manifest artifact} (follow-up
  session, unnumbered so the cross-references below keep their
  targets). A short session
  adding a third \code{ves-vocabulary} output: a canonical,
  golden-pinned machine-readable inventory (namespace, name, role,
  trust class, \code{cfg} feature, dependency list, invariant family)
  emitted alongside the Rust and Lean artifacts. Rewire
  \code{ves-tokens}'s build script to consume it instead of scraping
  \path{generated/tokens.rs}, closing 6(b), and hand the same file to
  WP-13 so \code{VesCore} can distinguish a grant-axiomatized
  proposition from a derived one without re-reading the Rust.
  \emph{(S; \code{standard}; high signal --- golden files again, and
  the two consumers are existing tests.)} Best done before WP-13
  begins, since retrofitting the distinction into an existing
  elaborator is more expensive than generating it now.
...
\item \textbf{[C] \code{VesCore} statement layer.} (Requires 5;
  preferably 6b, whose manifest tells the elaborator which
...
Items 1--7 are complete: lane A's Phase 0 (1, 2), the repository seeds
(3), and lane B's \code{ves-syntax}, \code{ves-vocabulary},
\code{ves-tokens}, and \code{ves-macros} (4, 5, 6, 7). The
token-identity property that makes \code{ves-syntax}'s hashes and
\code{ves-tokens}'s ZSTs mean anything at a real unsafe site is
therefore realized, not just designed. One session can start
immediately:
...
\item \textbf{Lane B: item 8, \code{ves-check}.} Now the sole gate on
  ...
  at all.
\item \textbf{Lane C: item 12, \code{VesCore}.} Unblocked by 5,
  ...
  nothing about \code{ves-macros} landing changes its readiness.
\end{itemize}

Item 6b should be slotted into lane B whenever there is a gap before 8
starts (or interleaved with it, since the two touch different crates),
and must precede 12 if 12 is to consume it. Item 8, then the
integration bootstrap (9), are the only things standing between the
present state and the Phase 1 deliverable, which is also the first
point at which the study's central hypothesis --- that scripts survive
this project's refactor velocity --- is actually under test rather
than under discussion.
