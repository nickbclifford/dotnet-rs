...
(the embedded layer) is now well underway: four of its five
\path{ves-proof} crates --- \code{ves-syntax}, \code{ves-vocabulary},
\code{ves-tokens}, and \code{ves-macros} --- are implemented and
tested, along with the 6b follow-up that gives \code{ves-tokens} (and,
later, \code{VesCore}) a golden-pinned vocabulary manifest instead of
a text-scraper. \code{ves-check} is the one remaining crate and the
sole gate on wiring this workspace into \dotnetrs{}; the sibling
...
lane B. Landing \code{ves-check} remains the cheapest experiment that
tests the whole
