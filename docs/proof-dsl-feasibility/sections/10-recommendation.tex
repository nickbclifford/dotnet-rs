...
differential harness covers seven fixtures rather than one. The
sibling repositories \path{ves-proof} and \path{ves-proof-lean} are
also seeded (WP-3) with their reserved package layout, but every
crate and package is still an inert scaffold --- no toolchain is
pinned in \path{ves-proof-lean}, and none of the five Rust crates has
logic beyond a marker constant. Phase 1 (the embedded layer,
starting with \code{ves-syntax}) is therefore the next and now the
only blocking action, and remains the cheapest experiment that tests
the whole thesis on real ground: uniquely among the phases, it yields
value even with no Lean involvement, since ghost tokens make premise
