...
\code{then}-expression verbatim, which \code{ves-macros} (commit
\code{85a6abc}) now realizes by splicing the parsed token stream
directly rather than reconstructing it, checked by five expansion-snapshot
fixtures in its own CI. What no hash can close is the
...
does not compile, so the registry cannot silently lag the code --- and
as of \code{ves-proof} commit \code{73e6dea} it exists: the generated
\code{grants} module is empty unless a registry is named, so the
default build can mint none of the four trusted constructors. The
registry \emph{reader}, however, is still a placeholder that matches
constructor names as substrings of the raw file, which fails open in
exactly the direction this subsection worries about --- a
commented-out entry grants trust. Replacing it, and fixing the
\code{TRUSTED.toml} schema so that each entry must name its falsifier
at registration time rather than by convention, is the first-listed
obligation of the integration bootstrap
(\cref{sec:decomposition-wps}, item 9). Even then
the ratchet requires enforcement: CI reports the grant count as a
