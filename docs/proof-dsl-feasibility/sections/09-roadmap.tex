...
\item[Phase 1 --- The embedded layer (4--7 weeks).] \textbf{In
  ...
  statement/subject hashing, golden corpus); the declarative
  vocabulary v0 (\textbf{done}, commit \code{ad67707}: 71 F1--F9
  constructors across 20 namespaces with trust and family annotations,
  ...
  \code{TrustGrant}s generated from \code{TRUSTED.toml} (\textbf{done,
  first pass}, commit \code{73e6dea}: the generated ZSTs, the
  exhaustive zero-size and \code{inline} matrices, and a grants module
  that is empty --- so no trusted premise is mintable --- unless a
  registry is supplied; the follow-up manifest work package (6b,
  \textbf{done}, commit \code{c23ede8}) later replaced its text-scraped
  constructor discovery with a golden-pinned vocabulary manifest, also
  handed to \code{VesCore}. The \code{TRUSTED.toml} schema and its real
  parser move to the integration bootstrap, which is where the file is
  actually authored and seeded from the accepted-risk boundaries;
  see \cref{sec:decomposition-wps}, items 6 and 9); \code{ves-macros}
  with token-identity snapshot tests (\textbf{done, first pass}, commit
  \code{85a6abc}: \code{proof!}, \code{proof\_impl!}, and \code{exempt!}
  expand over \code{ves-syntax}, splicing the protected tokens verbatim
  and confirmed by five expansion snapshots and three compile-fail
  fixtures; \code{contract!} is an identity passthrough pending the
  contract-layer package); \code{ves-check} in structural mode
  (\textbf{done}, commits \code{2c93b4b}/\code{e06a34b}/\code{da52ee5}/%
  \code{bb7ea0a}, 2026-08-03: extraction, structural rules, manifest
  comparison, and per-family ratchet budgets, all fixture-tested, 57
  tests passing; the standalone binary never links \code{ves-vocabulary}
  and reads the vocabulary manifest at runtime instead); scripts
  on every unsafe block in \crate{dotnet-value}, \crate{dotnet-utils},
  ...
  than a model. All five crates are in; the integration bootstrap that
  wires them into \dotnetrs{} (\cref{sec:decomposition-wps}, item 9) is
  now the critical path, and nothing in \dotnetrs{} changes until it
  lands (\cref{sec:decomposition-position}).
