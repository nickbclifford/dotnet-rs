...
**Central insight:** the "real frameworks need `Reflection.Emit`" fear is largely false. The BCL ships a **non-emitting interpreter** for `Expression.Compile` that activates when the runtime reports `RuntimeFeature.IsDynamicCodeSupported == false`. Because `dotnet-rs` already runs IL-bodied BCL code, flipping that one switch routes EF/LINQ/DI hot paths onto interpreter code we can already execute. This reframed EF Core from "blocked" to "reachable" — and it has since been carried all the way to **running**: the EF Core InMemory probe now executes end-to-end on `dotnet-rs` with `dotnet`-parity output (§4, §5), and the interpreter fallback proved sufficient with no `Reflection.Emit`.
...
4. **EF Core InMemory → SQLite** (Option B) — the headline engine-breadth target. **InMemory is now done** (runs with `dotnet` parity — §4/§5); SQLite (still open) adds filesystem + native P/Invoke.
...
**Loader/host — flat directory *or* a real host (Option A landed).** The low-level `--assemblies <FOLDER>` + entry `.dll` mode remains (`crates/dotnet-cli/src/lib.rs`), but host mode now reads `<app>.runtimeconfig.json` (roll-forward) and `<app>.deps.json` (multi-root + native probing) with **no `--assemblies` flag** (`crates/dotnet-assemblies/src/host.rs`), loading from app-base + shared framework + `~/.nuget/packages`. This is Option A (§4), landed 2026-06. NuGet *restore* is still delegated to the SDK; there is no hostfxr. *(This closes the gap this section originally described as open.)*
...
1. **`runtimeconfig.json` reader + roll-forward.** Pick the shared-framework version the way the host does. Test: run today's fixtures via their real `runtimeconfig.json` instead of the `--assemblies` flag. **✅ Done (2026-06, `supervised/nuget-host-runner`):** `crates/dotnet-assemblies/src/host.rs` parses `runtimeconfig.json` and implements all six roll-forward policies; `--assemblies` is now optional and host mode auto-detects from `<app>.runtimeconfig.json`.
2. **`deps.json` reader + multi-path probing.** Load managed assemblies from app-base + framework dir + `~/.nuget/packages` per the deps.json asset lists. Test: a project with one pure-managed `PackageReference` (e.g. `Newtonsoft.Json`) runs. **✅ Done (2026-06):** `deps.json` parser + `AssemblyLoader` multi-root probing (`probing_paths`, `add_scan_root`, `new_from_host`). **Verified:** the Newtonsoft.Json host-mode probe now matches stock `dotnet` exactly (`{"name":"test","value":42}`, exit 42).
3. **`dotnet-rs run <project-or-dir>` UX.** Detect a `.csproj`/published folder; if a project, shell out to `dotnet build` then host the output (mirror `dotnet run` = build-then-exec). Keep the existing low-level `--assemblies` mode for the test harness. *(Not yet implemented; `scripts/diff_run.sh` covers build-then-run for the differential harness.)*
4. **Native-library probing** wired to the existing P/Invoke loader (`crates/dotnet-pinvoke/`), so packages carrying native assets (later: SQLite) resolve. **✅ Done (2026-06):** host-derived native search dirs wired into `dotnet-pinvoke::NativeLibraries`/VM state; framework `.so`s resolve in host mode without manual copying.
5. **`configProperties` → feature switches**, including §3.1's `IsDynamicCodeSupported` and `System.Runtime.Loader.UseRidGraph`-style toggles, read from `runtimeconfig.json` and surfaced via `AppContext`. *(Not yet implemented; `IsDynamicCodeSupported=false` is still seeded unconditionally per §3.1.)*

**Out of scope / cannot run** (the runner now fails loudly on these, not weirdly — `probe_entry_kind` in `host.rs`, wired into `run_cli`):
- **NativeAOT publish** → a native executable with no IL; nothing for an interpreter to load. **Now detected** (ELF/Mach-O magic, or a PE with no CLI header) → explicit error, exit 1.
- **Single-file bundles** → require bundle extraction before the DLLs are visible. **Now detected** (bundle signature in the tail) → explicit error, exit 1. (Could be supported later by reusing the host's extraction logic, but defer.)
- **ReadyToRun (R2R)** images → *fine*: they still carry IL (a CLI header), and an interpreter just ignores the precompiled native code and interprets the IL; classified as Managed.
...
**Host-runner outcome (2026-06, `supervised/nuget-host-runner`).** Option A milestones 1, 2, and 4 are done and the fail-loud guard above is in place: `dotnet-rs <app.dll>` now runs framework-dependent apps with no `--assemblies` flag by reading `runtimeconfig.json` (roll-forward) and `deps.json` (multi-root + native probing), while `--assemblies` is unchanged. Verified on a 3-rung ladder: **rung 1 (fixtures)** ✅ full suite green in host mode; **rung 2 (Newtonsoft.Json)** ✅ stdout/exit parity with stock `dotnet`; **rung 3 (EF Core InMemory)** ✅ **now passes** — the InMemory probe (`tests/ef-probe/EfApp.dll`) prints `Hello` and exits 42 under both `dotnet` and `dotnet-rs`. Reaching parity took a sequence of engine fixes (generic-method-arg threading through `DbContextOptions..ctor`, `GetCustomAttributesData`/`NullabilityInfoContext`, by-ref value-type marshaling in `MethodInfo.Invoke` behind `IdentityMap.TryGetEntry`, and query-translation method identity); the two root-cause fixes are pinned by regression fixtures under `crates/dotnet-cli/tests/fixtures/reflection/`. The host path was never the blocker — EF (Option B) was a pure engine epic, now landed for the InMemory tier.

## 5. Option B — Entity Framework Core

**Thesis: EF Core is the single best *execution-engine* stress target in the ecosystem, and §3.1 made it feasible without `Reflection.Emit`.** One EF query exercises expression trees, deep generics, runtime reflection, custom attributes, `IEnumerable`/`IQueryable` plumbing, string/format machinery, dictionaries, and async — simultaneously. If EF's own functional test suite goes green, an enormous amount of the runtime has been transitively validated by code we didn't write.

**Status (2026-07): EF Core InMemory ✅ done.** The minimal probe (`tests/ef-probe/`) runs with `dotnet`-parity output (§4). §3.1's interpreter fallback proved sufficient — no `Reflection.Emit` was needed. Milestones 1–2 below are complete; 3–4 (InMemory breadth, then SQLite) remain open.
...
**Phased milestones:**
1. **Substrate:** §3.1 switch + verify §3.4 reflection/attributes. Probe: `Expression.Compile` interpreter smoke test (§3.1) and a reflective attribute read. **✅ Done.**
2. **EF InMemory minimal:** define a `DbContext` with one entity, `Add` + `SaveChanges` + a `Where/Select/ToList` query, assert results. This is the headline milestone — EF's core engine running on `dotnet-rs`. **✅ Done (2026-07):** the probe (`DbContext` + one `Blog` + `SaveChanges` + `First()`) runs with `dotnet` parity.
...
3. **`deps.json`/`runtimeconfig.json` parse-and-run (de-risks A).** Don't build the full host yet — write a throwaway: parse a real fixture's `*.deps.json` + `*.runtimeconfig.json`, derive the assembly list, feed the existing loader. *If it runs an existing fixture this way:* Option A milestone 1–2 is validated end-to-end. **Status (2026-06, `supervised/nuget-host-runner`): ✅ Completed and superseded by the real host.** Fixtures run via their real `runtimeconfig.json`/`deps.json` with no `-a` flag (Option A milestones 1–2 above); the full fixture suite passes in host mode.
4. **One pure-managed NuGet package (de-risks A).** Build a trivial app referencing `Newtonsoft.Json`, restore, then load `Newtonsoft.Json.dll` from `~/.nuget/packages` and round-trip a small object. Confirms multi-assembly NuGet loading with zero native deps. **Status (2026-06): ✅ Completed.** `Newtonsoft.Json 13.0.3` host-mode probe loads from `~/.nuget/packages` and serializes to full stdout parity with stock `dotnet`. (Reaching parity required several VM/reflection fixes — notably caching reflection `PropertyInfo` objects so `GetSerializableMembers` member discovery works.)
5. **EF InMemory smoke (de-risks B, the headline).** Minimal `DbContext`, one entity, `Add`+`SaveChanges`+`Where/ToList`. Run under `dotnet` and `dotnet-rs`; diff. Gated on probes 1–2. *This is the milestone that would prove the thesis.* **Status (2026-07): ✅ Done.** The probe (`tests/ef-probe/EfApp.dll`: `DbContext` + one `Blog` + `SaveChanges` + `First()`) prints `Hello` and exits 42 under both runtimes. The original `Generic index 0 out of bounds` failure in `DbContextOptions..ctor` and every downstream blocker (nullability conventions, identity-map by-ref marshaling, query-translation method identity) are fixed; §3.1's interpreter fallback carried it with no `Reflection.Emit`. **Thesis proven.** SQLite / InMemory breadth remain open (§5).
