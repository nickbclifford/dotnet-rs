...
**Host-runner outcome (2026-06, `supervised/nuget-host-runner`).** Option A milestones 1, 2, and 4 are done and the fail-loud guard above is in place: `dotnet-rs <app.dll>` now runs framework-dependent apps with no `--assemblies` flag by reading `runtimeconfig.json` (roll-forward) and `deps.json` (multi-root + native probing), while `--assemblies` is unchanged. Verified on a 3-rung ladder: **rung 1 (fixtures)** ✅ full suite green in host mode; **rung 2 (Newtonsoft.Json)** ✅ stdout/exit parity with stock `dotnet`; **rung 3 (EF Core InMemory)** ❌ blocked on a core generic-substitution engine bug in `DbContextOptions..ctor` (`Generic index 0 out of bounds`; details + localization in `docs/EF_GAP_BACKLOG.md`). The EF blocker is an engine gap, not a loader gap — the host path is complete; EF (Option B) is a separate epic gated on generic-method-arg threading.
...
5. **EF InMemory smoke (de-risks B, the headline).** Minimal `DbContext`, one entity, `Add`+`SaveChanges`+`Where/ToList`. Run under `dotnet` and `dotnet-rs`; diff. Gated on probes 1–2. *This is the milestone that would prove the thesis.* **Status (2026-06): ❌ Blocked — see `docs/EF_GAP_BACKLOG.md`.** The host loads EF and its closure, but execution fails in `DbContextOptions..ctor` with `Type resolution failed: Generic index 0 out of bounds (length 0)`: a generic *static-method* call (`ImmutableSortedDictionary.Create<TKey,TValue>`) resolves its method-generic args against an empty lookup. This is a core generic-substitution engine bug (not a loader/host gap), and EF will surface further blockers behind it — a separate epic.
