...
- [ ] 5.10 Rung-2 parity verification: re-run `bash scripts/diff_run.sh /tmp/nuget-probe/App.csproj` after all blocker fixes (5.4–5.9); exit code and stdout must match stock `dotnet`; if a new failure surfaces, diagnose, record here, and add a follow-up fix step [effort: default] — refs REVIEW.md#F-TEST-001
